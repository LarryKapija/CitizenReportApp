...
namespace CiudApp.Services
{
    public class GoogleMapsApiService : IGoogleMapsApiService
    {
        ...
        public static void Initialize(string paramGoogleMapsKey)
        {
            googleMapsKey = paramGoogleMapsKey;
        }
        ...
        public async Task<GooglePlaceAutoCompleteResult> GetPlaces(string text)
        {
            ...
            {
                var response = await httpClient.GetAsync($"api/place/autocomplete/json?input={Uri.EscapeUriString(text)}&key={googleMapsKey}").ConfigureAwait(false);
        ...
        public async Task<GooglePlace> GetPlaceDetails(string placeId)
        {
            ...
            using (var httpClient = CreateClient())
            {
                var response = await httpClient.GetAsync($"api/place/details/json?placeid={Uri.EscapeUriString(placeId)}&key={googleMapsKey}").ConfigureAwait(false);
                if (response.IsSuccessStatusCode)
                {
                    var json = await response.Content.ReadAsStringAsync().ConfigureAwait(false);
                    if (!string.IsNullOrWhiteSpace(json) && json != "ERROR")
